...
use App\Http\DB\ContactForm;
use App\Commands\Admin\ContactFormStoreCommand;
...
class ContactFormsController extends Controller
{
    ...
    public function __construct() {
        $this->view     = 'admin.general.contactforms.';
        $this->index    = 'admin/general/contactforms/index.';
        $this->create   = 'admin/general/contactforms/create.';
        $this->show    = 'admin/general/contactforms/show.';
    }
    ...
    public function show($id)
    {
        try {
            ...
            $data['lang']       = $this->show;
            $data['view']       = $this->view;
            $data['record']     = ContactForm::where('id', $id)->first();
            $data['index']      = '1';            
    ...
    public function destroy(Request $request, $id)
    {
        try {
            $data = $request->all();
            $data['id']         = $id;
            $result = $this->dispatch(new ContactFormStoreCommand($data, $request, 'delete'));
        ...
        try {
            ...
            return Datatables::of($list)
                    ...
                    ->addColumn('action', function($record) {
                        return '<a href="'.\URL::route('admin.contactforms.show', [ 'id' => $record->id ]).'" class="btn blue btn-icon-only btn-outline"><i class="fa fa-eye"></i></a>&nbsp;
                                <a href="javascript:;" onClick="deleteRow(\''.$record->id.'\');" id="'.$record->id.'" class="btn red-thunderbird btn-icon-only btn-outline" data-url="' . \URL::route('admin.contactforms.delete', ['id' => $record->id]) . '"><i class="fa fa-trash-o"></i></a>';       
                    })
                    ->make(true);
        } catch (Exception $e) {
