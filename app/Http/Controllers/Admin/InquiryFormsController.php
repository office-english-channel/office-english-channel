...
use App\Commands\Admin\InquiryFormStoreCommand;

class InquiryFormsController extends Controller
{
    public $view    = '';
    public $index   = '';
    public $create  = '';
    ...
    public function __construct() {
        $this->view     = 'admin.general.inquiryforms.';
        $this->index    = 'admin/general/inquiryforms/index.';
        $this->create   = 'admin/general/inquiryforms/create.';
        $this->show    = 'admin/general/inquiryforms/show.';
    }
    ...
    public function show($id)
    {
        try {
            ...
            $data['lang']       = $this->show;
            $data['view']       = $this->view;
            $data['record']     = InquiryForm::where('id', $id)->first();
            $data['index']      = '1';            
    ...
    public function destroy(Request $request, $id)
    {
        try {
            $data = $request->all();
            $data['id']         = $id;
            $result = $this->dispatch(new InquiryFormStoreCommand($data, $request, 'delete'));

            if ($result == 'done') {
                Session::flash('success', \Lang::get($this->index.'delete_success_msg') );
                return redirect(route('admin.inquiryforms.index'));
        ...
        try {
            ...
            return Datatables::of($list)
                    ...
                    ->addColumn('action', function($record) {
                        return '<a href="'.\URL::route('admin.inquiryforms.show', [ 'id' => $record->id ]).'" class="btn blue btn-icon-only btn-outline"><i class="fa fa-eye"></i></a>&nbsp;
                                <a href="javascript:;" onClick="deleteRow(\''.$record->id.'\');" id="'.$record->id.'" class="btn red-thunderbird btn-icon-only btn-outline" data-url="' . \URL::route('admin.inquiryforms.delete', ['id' => $record->id]) . '"><i class="fa fa-trash-o"></i></a>';       
                    })
                    ->make(true);
        } catch (Exception $e) {
