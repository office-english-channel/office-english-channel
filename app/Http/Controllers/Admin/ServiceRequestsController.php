...
use App\Commands\Admin\ServiceRequestStoreCommand;
...
class ServiceRequestsController extends Controller
{
    ...
    public function __construct() {
        $this->view     = 'admin.general.servicerequests.';
        $this->index    = 'admin/general/servicerequests/index.';
        $this->create   = 'admin/general/servicerequests/create.';
        $this->show     = 'admin/general/servicerequests/show.';
    }
    ...
    public function show($id)
    {
        try {
            ...
            $data['lang']       = $this->show;
            $data['view']       = $this->view;
            $data['record']     = ServiceRequest::where('id', $id)->first();
            $data['index']      = '1';            
    ...
    public function destroy(Request $request, $id)
    {
        try {
            $data = $request->all();
            $data['id']         = $id;
            $result = $this->dispatch(new ServiceRequestStoreCommand($data, $request, 'delete'));
        ...
        try {
            ...
            return Datatables::of($list)
                    ...
                    ->addColumn('action', function($record) {
                        return '<a href="'.\URL::route('admin.servicerequests.show', [ 'id' => $record->id ]).'" class="btn blue btn-icon-only btn-outline"><i class="fa fa-eye"></i></a>&nbsp;
                                <a href="javascript:;" onClick="deleteRow(\''.$record->id.'\');" id="'.$record->id.'" class="btn red-thunderbird btn-icon-only btn-outline" data-url="' . \URL::route('admin.servicerequests.delete', ['id' => $record->id]) . '"><i class="fa fa-trash-o"></i></a>';       
                    })
                    ->make(true);
        } catch (Exception $e) {
